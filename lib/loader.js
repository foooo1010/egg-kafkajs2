...
module.exports = app => {
  ...
  for (const name in kafkaConfig) {
    ...
    const lastPartitionMap = {};
    kafka[name] = {
      get client() {
        return client;
      },
      async send(topic, key, messages, partition, attributes = 0, timestamp = Date.now()) {
        if (!partition && partition !== 0) {
          const metaData = client.topicMetadata && client.topicMetadata[topic];
          app.logger.info('metaData:', metaData);
          if (metaData) {
            if (!lastPartitionMap[topic] && lastPartitionMap[topic] !== 0) {
              partition = 0;
            } else {
              partition = (lastPartitionMap[topic] + 1);
        ...
        lastPartitionMap[topic] = partition;
        app.logger.info('topic:', topic, 'partition:', partition, 'lastPartitionMap', lastPartitionMap);
